...
/**
 * `booking_groups` uchun RLS — `EnableRowLevelSecurityRatePlans1786900100000`dagi
...
export class EnableRowLevelSecurityBookingGroups1786960100000 implements MigrationInterface {
  name = 'EnableRowLevelSecurityBookingGroups1786960100000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `GRANT SELECT, INSERT, UPDATE, DELETE ON "booking_groups" TO "${APP_ROLE}"`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "booking_groups" DISABLE ROW LEVEL SECURITY`,
    );

    await queryRunner.query(
      `REVOKE SELECT, INSERT, UPDATE, DELETE ON "booking_groups" FROM "${APP_ROLE}"`,
    );
  }
}
